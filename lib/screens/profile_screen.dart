...
class ProfileScreen extends StatelessWidget {
  ...
  Course course = Course(0, '', 0, 0, 0, [0, 0, 0]);
  List<Course> courseList = [
    Course(0, '', 0, 0, 0, [0, 0, 0])
  ];
  Degree degree = Degree(0, '', [0, 0, 0]);
            ...
            builder: (context, snapView) {
              return StreamBuilder<List<Student>>(
                ...
                builder: (context, snapStudent) {
                  if (!snapStudent.hasData && !snapView.hasData && !snapshot.hasData) {
                    return Center(
                  ...
                  courseList = snapView.data;
                  Provider.of<FireStoreUni>(context).students = snapStudent.data;
                  return InnerDrawer(
                    key: _innerDrawerKey,
                    onTapClose: true,
                    swipe: true,
                                  ...
                                  children: <Widget>[
                                    ...
                                    Container(
                                      ...
                                      child: NotificationListener<OverscrollIndicatorNotification>(
                                        ...
                                        child: ListView.builder(
                                          physics: BouncingScrollPhysics(),
                                          itemCount: Provider.of<FireStoreUni>(context).students?.length ?? 0,
                                          itemBuilder: (BuildContext context, int index) {
                                            return ListTile(
                                              leading: Padding(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                              ...
                                              if (x == 0)
                                                StreamBuilder<List<Degree>>(
                                                  ...
                                                  builder: (context, snapDegree) {
                                                    ...
                                                    var len = snapDegree.data.length;
                                                    for (var i = 0; i < len; ++i) {
                                                      var length = snapDegree.data[i].Student_Ids.length;
                                                      for (var j = 0; j < length; ++j) {
                                                        if (snapDegree.data[i].Student_Ids[j] == student.Id) {
                                                          degree = snapDegree.data[i];
                                                        }
                                                      }
                                                    }
                                                    return Text(
                                ...
                                if (x == 2)
                                  Text(
                                    'Students taught by the faculty : ' + course.Student_Ids.length.toString(),
                                    style: TextStyle(fontSize: 20, color: Colors.black54),
                                  ),
                                if (x == 0)
                                  Text(
